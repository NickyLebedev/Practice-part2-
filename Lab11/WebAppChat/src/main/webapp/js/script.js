	...
	for(var j = 0;j < events.length; j++) {
		...
		if(index == appState.messageList.length) {
			...
			if(index!=finalList.length) {
				finalList[index].message = events[j].message;
			}
		}
		else {
			appState.messageList[index].message = events[j].message;
			...
			if(events[j].message == "Message was deleted") {
...
function deleteMes(evtObj) {
	var mess = evtObj.target.parentNode.parentNode.parentNode;
	var id = mess.attributes['data-id'].value;
	var mes = message("System message: ", "Message was deleted");
	mes.id = id;
	updateItem(mess, mes);
